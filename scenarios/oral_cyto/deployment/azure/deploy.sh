...
ORAL_CYTO_A_SAS_TOKEN=$(az storage blob generate-sas --account-name $AZURE_STORAGE_ACCOUNT_NAME --container-name $AZURE_ORAL_CYTO_A_CONTAINER_NAME --permissions r --name data.img --expiry $end --only-show-errors) 
export ORAL_CYTO_A_SAS_TOKEN=$(echo $ORAL_CYTO_A_SAS_TOKEN | tr -d \")
export ORAL_CYTO_A_SAS_TOKEN="?$ORAL_CYTO_A_SAS_TOKEN"

ORAL_CYTO_B_SAS_TOKEN=$(az storage blob generate-sas --account-name $AZURE_STORAGE_ACCOUNT_NAME --container-name $AZURE_ORAL_CYTO_B_CONTAINER_NAME --permissions r --name data.img --expiry $end --only-show-errors) 
export ORAL_CYTO_B_SAS_TOKEN=$(echo $ORAL_CYTO_B_SAS_TOKEN | tr -d \")
export ORAL_CYTO_B_SAS_TOKEN="?$ORAL_CYTO_B_SAS_TOKEN"
...
TMP=`echo $TMP | \
  jq '.azure_filesystems[2].azure_url = "https://" + env.AZURE_STORAGE_ACCOUNT_NAME + ".blob.core.windows.net/" + env.AZURE_MODEL_CONTAINER_NAME + "/data.img" + env.MODEL_SAS_TOKEN' | \
  jq '.azure_filesystems[2].mount_point = "/mnt/remote/model"' | \
  jq '.azure_filesystems[2].key.kid = "ModelFilesystemEncryptionKey"' | \
  ...
  jq '.azure_filesystems[2].key_derivation.label = "ModelFilesystemEncryptionKey"' | \
  jq '.azure_filesystems[2].key_derivation.salt = "9b53cddbe5b78a0b912a8f05f341bcd4dd839ea85d26a08efaef13e696d999f4"'`

TMP=`echo $TMP | \
  jq '.azure_filesystems[3].azure_url = "https://" + env.AZURE_STORAGE_ACCOUNT_NAME + ".blob.core.windows.net/" + env.AZURE_OUTPUT_CONTAINER_NAME + "/data.img" + env.OUTPUT_SAS_TOKEN' | \
  jq '.azure_filesystems[3].mount_point = "/mnt/remote/output"' | \
  jq '.azure_filesystems[3].key.kid = "OutputFilesystemEncryptionKey"' | \
  ...
  jq '.azure_filesystems[3].key_derivation.label = "OutputFilesystemEncryptionKey"' | \
