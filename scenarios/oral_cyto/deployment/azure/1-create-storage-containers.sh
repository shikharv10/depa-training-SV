...
# Check if the ORAL_CYTO_A container exists
CONTAINER_EXISTS=$(az storage container exists --name $AZURE_ORAL_CYTO_A_CONTAINER_NAME --account-name $AZURE_STORAGE_ACCOUNT_NAME --account-key $ACCOUNT_KEY --query "exists" --output tsv)

if [ "$CONTAINER_EXISTS" == "false" ]; then
    echo "Container $AZURE_ORAL_CYTO_A_CONTAINER_NAME does not exist. Creating it now..."
    az storage container create --resource-group $AZURE_RESOURCE_GROUP --account-name $AZURE_STORAGE_ACCOUNT_NAME --name $AZURE_ORAL_CYTO_A_CONTAINER_NAME --account-key $ACCOUNT_KEY
fi

# Check if the ORAL_CYTO_B container exists
CONTAINER_EXISTS=$(az storage container exists --name $AZURE_ORAL_CYTO_B_CONTAINER_NAME --account-name $AZURE_STORAGE_ACCOUNT_NAME --account-key $ACCOUNT_KEY --query "exists" --output tsv)

if [ "$CONTAINER_EXISTS" == "false" ]; then
    echo "Container $AZURE_ORAL_CYTO_B_CONTAINER_NAME does not exist. Creating it now..."
    az storage container create --resource-group $AZURE_RESOURCE_GROUP --account-name $AZURE_STORAGE_ACCOUNT_NAME --name $AZURE_ORAL_CYTO_B_CONTAINER_NAME --account-key $ACCOUNT_KEY
fi
